if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('service-worker.js').catch((err) => {
      alert(
        'There has been a problem with Service Worker registration. This application may require internet connection to function properly.',
      );
    });
  });
}
...
const favoriteColorContainers = document.querySelectorAll(
...
adjustColors = (hue, saturation, lightness) => {
  ...
  setCustomProperty(
    '--saturation-value',
    hslValuesToNotation(hue, saturation, 60),
  );
  if (lightness <= 30) {
    setCustomProperty('--secondary-color', hslValuesToNotation(0, 0, 73));
  } else {
    setCustomProperty('--secondary-color', hslValuesToNotation(44, 10, 13));
  }
...
changeHue = (difference) => {
  let { hue, saturation, lightness } = hslNotationToValues(
    getCustomProperty('--screen-light'),
  );
...
selectFavoriteColor = (element) => {
  ...
  const { hue, saturation, lightness } = hslNotationToValues(
    getCustomProperty(favoriteColorNumber),
  );
  adjustColors(hue, saturation, lightness);
...
changeBtnsVisibility = () => {
  ...
  document
    .querySelector('.palette__icon')
    .classList.toggle('palette__icon--hide');
  if (
    document.querySelector('.help__icon').classList.contains('help__icon--hide')
  ) {
...
swipe = () => {
  if (paletteContainer.classList.contains('palette__container--show')) {
    return;
  }
  let XOffset = touchEndX - touchStartX;
  let YOffset = touchEndY - touchStartY;
  if (Math.abs(XOffset) < 80 && Math.abs(YOffset) < 80) {
    return;
  }
...
document
  .querySelector('.palette__container')
...
(function () {
  if (localStorage.getItem('--screen-light')) {
    let { hue, saturation, lightness } = hslNotationToValues(
      localStorage.getItem('--screen-light'),
    );
    adjustColors(hue, saturation, lightness);
  }
  if (localStorage.getItem('hide-btns')) {
    ...
    document
      .querySelector('.palette__icon')
      .classList.add('palette__icon--hide');
  }
  for (let element of favoriteColorContainers) {
    if (localStorage.getItem(element.dataset.propertyName)) {
      setCustomProperty(
        element.dataset.propertyName,
        localStorage.getItem(element.dataset.propertyName),
      );
    }
  }
})();
